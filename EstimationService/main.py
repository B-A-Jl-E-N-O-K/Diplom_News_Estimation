...
if __name__ == "__main__":
    # Kafka data
    KAFKA_ADDRESS = os.environ['KAFKA_SERVER_ADDRESS']
    # KAFKA_ADDRESS = "kafka1:19091"
    KAFKA_TOPIC = 'news'
    KAFKA_LOGS = 'logs'

    try:
        producer_kafka = KafkaProducer(bootstrap_servers=[KAFKA_ADDRESS], security_protocol="SASL_PLAINTEXT",  
            sasl_mechanism='PLAIN',  
            sasl_plain_username='user',  
            sasl_plain_password='user',
            value_serializer=lambda x: json.dumps(x).encode('utf-8'))
    except (Exception, Error) as error:
        print('Error in create kafka connection:', error)
        raise

    def logs_out(type, mes):
        log = {type: mes}
        producer_kafka.send(KAFKA_LOGS, log)
        return 0
    ...
    for_regex_industry = {
        'finance': ['ипотека', 'sber', '/^сбер$/', 'сбербанк', 'банк', 'кредит', 'депозит', 'мкб', 'московский кредитный банк', 'cbom', 'vtbr', 'втб', 'финансовый', 'финансы', 'биржа', 'moex', 'tcsg', 'тинькофф', 'ценные бумаги', 'ценная бумага', 'облигация', 'облигации', 'паевый', 'фонд', 'котировка'],
        'mining': ['poly', 'plzl', 'полиметалл', 'полюс золото', 'золото', 'pogr', 'petropavlovsk plc', 'petropavlovsk', 'железо', 'сталь', 'северсталь', 'чугун', 'ммк', 'магнитогорский металлургический комбинат', 'magn', 'северсталь', 'chmf', 'nlmk', 'новолипецкий металлургический комбинат', 'металл', 'rual', 'rusal', 'uc rusal','русал', 'gmkn', 'норникель', 'норильский никель', 'никель', 'алюминий', 'палладий', 'олово', 'медь', 'глинозем'],
        'energy':['gazp', 'газпром', 'rosn', 'роснефть', 'сургутнефтегаз', 'sngs', 'газ', 'нефть', 'нефтепровод', 'газопровод', 'татнефть', 'tatn', 'лукойл', 'lkoh', 'транснефть', 'trnfp', 'новатэк', 'nvtk', 'интер рао', 'irao', 'русгидро', 'hydr', 'элетричество', 'гэс', 'тэц', 'энергетика', 'фск еэс', 'fees'],
        'telecom_it_tech': ['мтс', 'mtss', 'ростелеком', 'rtkm', 'сеть', 'связь', 'интернет', 'провайдер', 'машинный обучение', 'цифровой технология', 'облако', 'вк', 'мэйл', 'вконтакте', 'mail', 'vk', 'vkco', 'яндекс', 'yandex', 'yndx', 'м.видео', 'mvideo', 'm. video', 'dns', 'citilink', 'смартфон', 'ноутбук', 'телевизор', 'бытовой', 'техника'],
        'real_estate': ['недвижимость', 'новостройка', 'ипотека', 'лср', 'lsrg', 'пик', 'pikk'],
        'consume_retail': ['напиток', 'вода', 'абрау', 'дюрсо', 'abrd', 'алкоголь', 'вино', 'еда', 'пищевой', 'пища', 'магнит', 'перекресток', 'пятерочка', 'продукт', 'магазин'],
        'research': ['исследование', 'наука', 'научный', 'nauk', 'нпо'],
        'medicine': ['лекарство', 'аптека', 'лечение', 'медицина', 'больница'],
        'aircraft': ['авиация', 'аэрофлот', 'aflt', 'aeroflot', 'самолет']
    }

    industries = for_regex_industry.keys()

    # DB data from env
    user_db = os.environ['DATASOURCE_USERNAME']
    password_db = os.environ['DATASOURCE_PASSWORD']
    host_db = os.environ['DATASOURCE_HOST']
    port_db = os.environ['DATASOURCE_PORT']
    name_db = os.environ['DATASOURCE_DB_NAME']

    # DB data without env
    # user_db = "postgres"
    # password_db = "postgres"
    # host_db = "estimation_db"
    # port_db = "5432"
    # name_db = "news_estimation_db"
    ...
    try:
        ...
        logs_out('warning', str(error))
    finally:
        if connection:
                cursor.close()
                connection.close()

    table_name = 'est_news_table'
...
def main():
    ...
    while 1:
        ...
        if is_taken_news == 0:
            ...
            print(f"Collecting news time (iteration {supply_counter}): {collecting_time} seconds", file=sys.stdout)
            if collecting_time > 120:
                print(f"Warning: Collecting news time above limit", file=sys.stdout)
        ...
        if supply_counter % (1) == 0: # 1 week interval
            ...
            collecting_time = round(time.time() - start_time)
            print(f"Making ML model (iteration {supply_counter}): {collecting_time} seconds", file=sys.stdout)
            if collecting_time > 300:
                print(f"Warning: Making ML model time above limit", file=sys.stdout)
            time.sleep(60)
...
def est_news():
    ...
    if(news_df is None):
        logs_out('error', 'Error in lemmatize file')
        return -1
...
def save_db(df):
    ...
    try:
        cursor = connection.cursor()
        for index, row in df.iterrows(): 
            insert_query = f"INSERT INTO {table_name} (link, datetime, title_origin, company) VALUES ('{row['link']}', '{str(row['datetime'])}', '{row['title origin']}', '{row['company']}')"
...
def results_out(df):
    ...
    df = df.reset_index()
    df = df.rename(columns={'title out': 'title_out'})
    df = df.rename(columns={'class': 'class_news'})
    ...
    producer_kafka.send(KAFKA_TOPIC, deliver)

    print('Saved news: ' + str(len(df)), file=sys.stdout)
...
def create_model(df):
    ...
    with open("ml/columns.txt", "w", encoding='UTF-8') as file:
    ...
    joblib.dump(estimator, "ml/ML_model.joblib")
